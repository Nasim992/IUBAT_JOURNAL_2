...
function total_assopaper($email){
    global $dbh;
    $query = "SELECT COUNT(*) as total_rowsrev FROM editortable where primaryemail = '$email'  and associateeditor IS NOT NULL and accepted IS NOT NULL";
    $stmt = $dbh->prepare($query);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
...
function total_acapaper($email) {
    global $dbh;
    $query = "SELECT COUNT(*) as total_rowsrev FROM editortable where primaryemail = '$email'  and academiceditor IS NOT NULL and accepted IS NOT NULL";
    $stmt = $dbh->prepare($query);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
